...
logging.basicConfig(format="[%(asctime)s] [%(filename)s] %(message)s",
                    level=logging.INFO, filename='./reduced.log',
                    )
...
is_random = 'ordered'
print(
    "target:%d,source:%d,%s, seq_length: %d, batch_size: %d, origin_learning_rate: %f, hidden_size: %d, num_layers: %d, weight_decay: %f, patience: %d, threshold: %f" % (
    target_variable,source_variable,is_random, seq_length, batch_size, origin_learning_rate, hidden_size, num_layers, weight_decay, patience,
    threshold))
logging.info(
    'target:{},source:{},{}, seq_length: {}, batch_size: {}, origin_learning_rate: {}, hidden_size: {}, num_layers: {}, weight_decay: {}, patience: {}, threshold: {}'.format(
        target_variable,source_variable,is_random, seq_length, batch_size, origin_learning_rate, hidden_size, num_layers, weight_decay, patience,
        threshold
    ))
...
def sliding_windows(data):
    ...
    while i+seq_length<data.shape[0]:
        ...
        past_x=np.delete(past_x,source_variable,1)
...
print("merge_x ", merge_x.shape)  # merge_x  (990, 10, 9)
...
while num_of_epoch < max_of_epoch:
    ...
    # if os.path.exists('./reduced.pkl'):
    #     last_best_model = torch.load('./reduced.pkl')
    ...
    #         torch.save(best_model, './reduced0001.pkl')
    #         print("source ",source_variable)
    #         logging.info('source:{}'.format(source_variable))
    # else:
    torch.save(best_model, './model_trained/without_weight_decay/hidden'+str(hidden_size)+'reduced.pkl')
