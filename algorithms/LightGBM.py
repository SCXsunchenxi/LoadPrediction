...
class LightGBM(object):
    ...
    '''''
    def train(self):
        print('[*******************************************]LightGBM MODEL TRAINING PROCESS')
        ...
        for m in range(create_model_number):
            ...
            # 有数据，使用去除多重共线性的数据训练模型
                ...
                for i in range(strain_number):
                    ...
                    models.append(clf) # 训练好的模型添加到模型列表中
                    result.append(evals_result)
                    y_pre = models[i].predict(X_test)
                    # 计算模型的精度
                    r=explained_variance_score(y_pre, Y_test[:, i])
                    if(r>0.5):
                        acc=acc+r
                    else: # 去除无法训练的模型
                        r_number=r_number-1
                ...
                self.model_list.append(models) # 保存子模型
                if(r_number>0):
                    acc = acc / r_number
    ...
    def save_model(self):
        ...
        # 按照子模型和应变划分来保存模型
        for i in range(create_model_number):
            for j in range(self.strain_number):
                if (self.data_exist[i]==0): # 没有训练则不保存------------
                    continue
                path = os.path.join(self.save_dir, f'{str(i)}_{str(j)}.pkl')
                joblib.dump(self.model_list[i][j], path)
    ...
    def load_model(self,pre_strain_number):
        ...
        # 加载模型的预测因变量特征
        mean_std_path = os.path.join('static/model/决策树(LightGBM)', str(self.idx), self.plane, '0_mean_sd.csv')
        ...
        for i in range(create_model_number):
            models=[]
            for j in range(pre_strain_number):
                if (self.data_exist[i]==0): # 没有训练则不加载------------
                    models.append('None')
                    continue
                path = os.path.join(self.save_dir, f'{str(i)}_{str(j)}.pkl')
    ...
    def restore_value(self,data,m):
        ...
        # 获取均值方差文件
        mean_std_path = os.path.join('static/model/决策树(LightGBM)', str(self.idx), self.plane, str(m) + '_mean_sd.csv')
        mean_std = pd.read_csv(mean_std_path, encoding='utf-8')
        # 均值
    ...
    def test(self, data_dir,record_idx):
        print('[*******************************************]LightGBM MODEL TEST PROCESS')
        ...
        if(self.model_number==0):
            ...
            for i in range(model_number):
                ...
                # 应变预测
                pred = [model.predict(X).tolist() for model in self.model_list[i]]
        ...
        # 天空点多模型/整体模型的预测方法
            ...
            for m in range(self.model_number):
                ...
                # 预测
                pred = [model.predict(X).tolist() for model in self.model_list[m]]
    ...
    def explain(self):
        ...
        for s in range(len(self.target_col)):
            ...
            for m in range(create_model_number):
                ...
                # 计算模型的特征重要度
                imps = model[s].feature_importance()
...
if __name__ == "__main__":
    for plane in ['P123', 'P126', 'P125', 'P127', 'P124']:
        model = LightGBM('决策树(LightGBM)', 1, 'static/data/processed_data/train/', plane, 4, 0, 11)
